...
export async function getAddrRows(keys, callbacks = {}) {
  let rows = []
  ...
  for (let [pub, priv] of keys) {
    ...
    getStatusCN(pub)
      .then(s => {
        $d.getElementById(`stake_${pub}`).innerHTML = s
        if (callbacks.status) {
          callbacks.status()
        }
      })
