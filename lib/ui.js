  ...
  if (balance?.TotalBalance) {
    return [
      `
        ...
        ${balanceEl(balance.TotalActiveBalance)}
...
export function formatWalletName(n) {
  return n.replace(KEY_PREFIX, '')
    .replaceAll('_', ' ')
}
...
export async function getAddrRows(container, keys, state = {}) {
  ...
  for (let [pub, priv, walletName] of keys) {
    ...
    tds[1].innerHTML = `<strong>${pub}</strong><br/><sup>${formatWalletName(walletName)}</sup>`
...
export async function getStakeRows(container, keys, state = {}) {
  ...
  for (let [pub, priv, walletName] of keys) {
    ...
    if (!foundRow) {
      ...
      row.innerHTML = `
        <${colType}></${colType}>
        <${colType} class="addr-col"></${colType}>
        <${colType} class="dash-addr-balance-col"></${colType}>
    ...
    getStatusCN(pub)
      .then(status => {
        let addrCol = row.querySelector(`.addr-col`)
        ...
        if (status) {
          ...
          addrCol.innerHTML = `<a href="https://app.crowdnode.io/FundsOpen/${pub}" target="_blank">${pub}</a><br/><sup>${formatWalletName(walletName)}</sup>`
...
export async function getCurrencies(symbol) {
  let rateUrl = new URL(
    'https://rates.ctx.com/rates?source=ctx'
  )
  if (symbol) {
