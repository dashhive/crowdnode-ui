      ...
      .then(([el, b, d]) => {
        ...
        if (dashFiat?.price) {
          ...
          row.querySelector(`.crowdnode-balance-col`).insertAdjacentHTML(
            'beforeend',
            `${balanceWithSymbol} - ${dividendWithSymbol}`
          )
        }
...
export async function getCurrencies(symbol) {
  ...
  return await fetch(
    ...
    .then(async list => await list.filter(({ baseCurrency }) => baseCurrency.toUpperCase() === 'DASH'))
  }
...
export async function updateFiatDisplay(el, symbol) {
  ...
  el.insertAdjacentHTML(
    'beforeend',
    `<span>1 ${fiat.baseCurrency} = ${priceWithSymbol}</span>`
  )
