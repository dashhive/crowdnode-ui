...
export function isStoreEncrypted() {
  for (const [key, value] of Object.entries(store)) {
    if (
      key.startsWith(KEY_PREFIX) &&
      value.includes(':')
    ) {
      console.log(`At least one key is encrypted.
      ${key}: ${value}`);
      return true
    }
  }
...
export async function getStoredKeys(pass) {
  ...
  let wallets = Object.keys(store).filter(
...
export async function getStoredAccounts() {
  let $s = store
  let wallets = Object.keys($s).filter(
    k => k.startsWith(KEY_PREFIX)
  )
  let accounts = JSON.parse(await $s.getItem('accounts')) || {}
  ...
  if (Object.keys(accounts).length < wallets.length) {
    for (let wkey in wallets) {
      let walletKey = wallets[wkey]
      let walletKeys
      let acctAddrs = accounts[walletKey] || []

      console.log('getStoredAccounts wallet foreach', walletKey, acctAddrs)

      if (acctAddrs.length === 0) {
        walletKeys = await generateRecoveryPhrase(
          $s[walletKey], // TODO: needs to account for encryption
        )
        accounts[walletKey] = [...acctAddrs]
        if (!acctAddrs.includes(walletKeys.address)) {
          accounts[walletKey].push(walletKeys.address)
        }
      }

      console.log('getStoredAccounts walletKeys', walletKeys.address)
    }

    console.log('getStoredAccounts accounts', accounts)
